# -*- coding: utf-8 -*-
"""
    Original: https://github.com/morgil/madgwick_py/blob/master/madgwickahrs.py
...
class MadgwickAHRS:
    ...
    def update(self, gyroscope, accelerometer, magnetometer=None):
        ...
        if self.last_upd != 0:
            curr_time = time.time()
            self.samplePeriod = curr_time - self.last_upd
            self.last_upd = curr_time
            
        if magnetometer is None:
            self.update_imu(gyroscope=gyroscope, accelerometer=accelerometer)
        q = self.quaternion
